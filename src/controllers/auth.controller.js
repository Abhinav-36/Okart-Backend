...
const register = catchAsync(async (req, res) => {
  ...
  let newUser = await userService.createUser(userDetails);
  let access =await tokenService.generateAuthTokens(newUser);
  res.status(201).json({user: newUser,tokens: access});
});
